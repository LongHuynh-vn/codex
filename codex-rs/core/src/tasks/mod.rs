...
impl Session {
    ...
    pub async fn on_task_finished(
        ...
        // Goal-free structural nudge (O31): covers the "all children delivered,
        // idle forever" failure without relying on the old auto-armed
        // orchestration goal crutch.
        self.maybe_nudge_gemini_orchestration_idle(turn_context.as_ref(), emitted_final_answer)
            .await;
        if let Err(err) = self
